					...
					<div class="payment_details_right_col saved-payment-list">
						...
						<c:forEach items="${paymentInfos}" var="paymentInfo">
							<div class="saved-payment-list-entry">
								<form action="/ankitstorefront/checkout/multi/select-payment-method" method="GET">
								...
								<form action="/ankitstorefront/checkout/multi/remove-payment-method" method="POST" class="remove-payment-item-form">
					...
					<div class="payment_details_left_col">
								...
								<div class="form_field-elements">
									...
									<div class="form_field-input">
										<sec:authorize ifNotGranted="ROLE_ANONYMOUS">
											<formElement:formCheckbox idKey="checkout.multi.sop.savePaymentInfo" labelKey="checkout.multi.sop.savePaymentInfo" path="savePaymentInfo" labelCSS="add-address-left-label" mandatory="false"/>
										</sec:authorize>
									</div>
							</div>
						</div>
						...
						<div class="payment_details_left_col-billing">
							...
							<c:if test="${cartData.deliveryItemsQuantity > 0}">
								<formElement:formCheckbox idKey="useDeliveryAddress" labelKey="checkout.multi.sop.useMyDeliveryAddress" path="useDeliveryAddress" labelCSS="add-address-left-label" mandatory="false"/>
								<input type="hidden" id="useDeliveryAddressFields" name="orderPage_receiptResponseURL" data-firstname="${deliveryAddress.firstName}" data-lastname="${deliveryAddress.lastName}"
													data-line1="${deliveryAddress.line1}" data-line2="${deliveryAddress.line2}" data-town="${deliveryAddress.town}" data-postalcode="${deliveryAddress.postalCode}"
													data-countryisocode="${deliveryAddress.country.isocode}" data-regionisocode="${deliveryAddress.region.isocodeShort}"/>
							</c:if>
							<input type="hidden" value="${silentOrderPageData.parameters['billTo_email']}" class="text" name="billTo_email" id="billTo_email">
							<address:billAddressFormSelector supportedCountries="${countries}" regionsForCountries="${regionsForCountries}"/>
						</div>


						<div class="save_payment_details">
							<span class="clear_fix">
								<button data-theme="c" class="form submit_silentOrderPostForm"><spring:theme code="Submit" text="Submit"/></button>
							</span>
						</div>
						</form:form>
					</div>
