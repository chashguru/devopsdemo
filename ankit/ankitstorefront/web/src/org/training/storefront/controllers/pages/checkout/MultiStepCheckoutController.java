...
public class MultiStepCheckoutController extends AbstractCheckoutController
{
	...
	public List<CheckoutSteps> addCheckoutStepsToModel(final HttpServletRequest request)
	{
		...
		checkoutSteps.add(new CheckoutSteps("deliveryAddress", "/checkout/multi/choose-delivery-address"));
		checkoutSteps.add(new CheckoutSteps("deliveryMethod", "/checkout/multi/choose-delivery-method"));
		checkoutSteps.add(new CheckoutSteps("paymentMethod", "/checkout/multi/choose-payment-method"));
	...
	public String doChooseDeliveryAddress(final Model model) throws CMSItemNotFoundException
	{
		...
		if (hasNoDeliveryAddress() && getUserFacade().isAddressBookEmpty())
		{
			return REDIRECT_URL_ADD_DELIVERY_ADDRESS;
		}

		// set the default delivery address and calculate
	...
	public String doChoosePaymentMethod(final Model model) throws CMSItemNotFoundException
	{
		...
		// Try to set the default paymentInfo
		...
		final CartData cartData = getCheckoutFacade().getCheckoutCart();
		final CCPaymentInfoData selectedPaymentInfo = cartData.getPaymentInfo();
